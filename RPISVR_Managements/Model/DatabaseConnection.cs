                        ...
                        using (MySqlDataReader reader = cmd.ExecuteReader())
                        {
                            while (reader.Read())
                            {
                                Student_Info students = new Student_Info
                                {
                                    ...
                                    //ProfileImageBytes = reader.GetByte("ProfileImageBytes"),
                                    ...
                                    //Stu_Image_Degree_Bytes = reader.GetString("Stu_Image_Degree_Bytes"),
                                ...
                                {
                                    byte[] imageBytes = (byte[])reader["stu_image_source"];

                                    // Convert byte array to BitmapImage for UI display
                                    students.Stu_Image_Source = ConvertBytesToImage(imageBytes);
                                    students.ProfileImageBytes = imageBytes;  // Keep the byte array for further use
                                }

                                //Stu_Images_Degree
                                if (!reader.IsDBNull(reader.GetOrdinal("stu_image_degree_source")))
                                {
                                    byte[] imageBytes = (byte[])reader["stu_image_degree_source"];

                                    // Convert byte array to BitmapImage for UI display
                                    students.Stu_Image_Degree_Source = ConvertBytesToImage(imageBytes);
                                    students.Stu_Image_Degree_Bytes = imageBytes;  // Keep the byte array for further use
                                }

                                //Stu_ImageBirth_Cert
                                if (!reader.IsDBNull(reader.GetOrdinal("stu_image_birth_cert_source")))
                                {
                                    byte[] imageBytes = (byte[])reader["stu_image_birth_cert_source"];

                                    // Convert byte array to BitmapImage for UI display
                                    students.Stu_ImageBirth_Cert_Source = ConvertBytesToImage(imageBytes);
                                    students.Stu_ImageBirth_Cert_Bytes = imageBytes;  // Keep the byte array for further use
                                }

                                //Stu_Image_IDNation
                                if (!reader.IsDBNull(reader.GetOrdinal("stu_image_id_nation_source")))
                                {
                                    byte[] imageBytes = (byte[])reader["stu_image_id_nation_source"];

                                    // Convert byte array to BitmapImage for UI display
                                    students.Stu_ImageIDNation_Source = ConvertBytesToImage(imageBytes);
                                    students.Stu_ImageIDNation_Bytes = imageBytes;  // Keep the byte array for further use
                                }

                                //Stu_ImagePoor_Card
                                if (!reader.IsDBNull(reader.GetOrdinal("stu_image_poor_card_source")))
                                {
                                    byte[] imageBytes = (byte[])reader["stu_image_poor_card_source"];

                                    // Convert byte array to BitmapImage for UI display
                                    students.Stu_ImagePoor_Card_Source = ConvertBytesToImage(imageBytes);
                                    students.Stu_Image_Poor_Card_Bytes = imageBytes;  // Keep the byte array for further use
                                }
